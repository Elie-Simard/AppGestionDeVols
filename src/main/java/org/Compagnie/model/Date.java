...
import org.Compagnie.util.UtilVue;

import java.io.Serializable;
import java.time.LocalDate;

import static org.Compagnie.util.UtilVue.ajouterCaractereGauche;
...
public class Date implements Serializable {
    ...
    public static String validerDate(int jour, int mois, int an, boolean etat[]) {
        ...
        if (etat[1]) {
            ...
            message += "Le Mois " + tabMois[mois] + " est invalide";
        }
    ...
    public static Boolean validerDateReservation(Date dateReservation) {
        ...
        if (dateReservation.getMois() < dateActuelle.getMonthValue()
                && dateReservation.getAn() <= dateActuelle.getYear()) {
            etatDate = false;
            errorMessage("Le mois de réservation doit être supérieure ou égale à la date actuelle");
        }

        if (etatDate) {
            if (dateReservation.getJour() < dateActuelle.getDayOfMonth()
                    && dateReservation.getMois() <= dateActuelle.getMonthValue()
                    && dateReservation.getAn() <= dateActuelle.getYear()) {
                etatDate = false;
    ...
    public String toString() {
        ...
        leJour = ajouterCaractereGauche('0', 2, this.jour + "");
        leMois = ajouterCaractereGauche('0', 2, this.mois + "");
